...
void RunAnalysis( double pmin, double pmax, double etamin, double etamax)
{
...
TString infile = Form("../Output/P_%0.0lf_%0.0lf_Eta_%0.1lf_%0.1lf__Beast_FastSimEval.root", pmin, pmax, etamin, etamax);
...
//Defining Histograms and variables
	...
	TH2D *h2_momRes_vs_mom = new TH2D("h2_momRes_vs_mom", "(reco_p - truth_p)/truth_p vs Momentum", 41, -0.5, 40.5, 1000, -1, 1); 
	TH2D *h2_momRes_vs_eta = new TH2D("h2_momRes_vs_eta", "(reco_p - truth_p)/truth_p vs Eta", 70, -3.5, 3.5, 1000, -1, 1); 
	
	TH1D *h_ptRes = new TH1D("h_ptRes", "(reco_pt - truth_pt)/truth_pt", 1000, -1, 1);
	
	TH2D *h2_ptRes_vs_pt = new TH2D("h2_ptRes_vs_mom", "(reco_pt - truth_pt)/truth_pt vs Pt", 41, -0.5, 40.5, 1000, -1, 1); 
	TH2D *h2_ptRes_vs_eta = new TH2D("h2_ptRes_vs_eta", "(reco_pt - truth_pt)/truth_pt vs Eta", 70, -3.5, 3.5, 1000, -1, 1); 
...
for (int i = 0; i < nentries; i++)
{
	...
	h_momRes->SetNameTitle(Form("h_momRes_P_%0.1f_%0.1lf_Eta_%0.1lf_%0.1lf", pmin, pmax, etamin, etamax), Form("(reco_p - truth_p)/truth_p for tracks with pt between %0.1f - %0.1lf GeV/c and %0.1lf < eta < %0.1lf", pmin, pmax, etamin, etamax) );
	
	h_ptRes->SetNameTitle(Form("h_ptRes_P_%0.1f_%0.1lf_Eta_%0.1lf_%0.1lf",pmin, pmax, etamin, etamax), Form("(reco_pt - truth_pt)/truth_pt for tracks with pt between %0.1f - %0.1lf GeV/c and %0.1lf < eta < %0.1lf", pmin, pmax, etamin, etamax) );
